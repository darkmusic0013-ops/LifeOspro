...
const initialFinance = { salary: 50000, savingGoal: 8000, expenses: [
  ...
  { id: 'e2', name: 'Comida', amount: 4000, category: 'Mensual', type: 'Gasto' },
...
export default function Home(){
  ...
  useEffect(() => { try { window.localStorage.setItem(financeStorageKey, JSON.stringify(finance)); } catch {} }, [finance]);
  useEffect(() => { try { window.localStorage.setItem(ideaStorageKey, JSON.stringify(ideas)); } catch {} }, [ideas]);
  useEffect(() => { try { window.localStorage.setItem(mealStorageKey, JSON.stringify(meals)); } catch {} }, [meals]);
  useEffect(() => { try { window.localStorage.setItem(profileStorageKey, JSON.stringify(profile)); } catch {} }, [profile]);
  ...
  const savingProgress = pct(Math.max(balance,0), Number(finance.savingGoal||0));
  const money = [['Salario',fmt(finance.salary)],['Gastos',fmt(totalExpenses)],['Disponible',fmt(balance)]];
  ...
  function saveTask(){ if(!taskDraft.title.trim()) return; editingTaskId ? setTasks(tasks.map(t => t.id === editingTaskId ? { ...t, ...taskDraft } : t)) : setTasks([{ ...taskDraft, id: String(Date.now()), done: false }, ...tasks]); closeTaskForm(); setActiveTab('Tareas'); }
  ...
  const dashboard = <>
    <section className='hero'><div className='card heroCard'><div className='spark'>✨</div><span className='pill'>Enfoque de hoy</span><h1>Ejecuta tu día con claridad.</h1><p className='muted'>LifeOS convierte tus pendientes, metas, finanzas e ideas en una ruta visual para saber qué hacer ahora y qué viene después.</p><div className='grid'><div className='mini active'><b>{pendingTasks.length}</b><span>Pendientes</span></div><div className='mini'><b>{todayTasks.length}</b><span>Para hoy</span></div><div className='mini'><b>{fmt(finance.savingGoal)}</b><span>Ahorro meta</span></div><div className='mini'><b>{goals.length}</b><span>Metas</span></div></div></div><div className='card focus'><div className='focusTop'><h3>Prioridad inmediata</h3><span className='pulse'>En vivo</span></div>{topTasks.map((t,i)=><div className='row taskRow' key={t.id}><div className='rank'>{i+1}</div><div><b>{t.title}</b><small>{t.category} · {t.due}</small></div><span className={'priority '+(t.priority==='Alta'?'alta':t.priority==='Media'?'media':'baja')}>{t.priority}</span></div>)}</div></section>
    <section className='modules'><div className='card now'><h3>Qué debo hacer ahora</h3><p className='muted'>Ordenado por impacto y fecha.</p><ol>{topTasks.map(t=><li key={t.id}>{t.title}</li>)}</ol></div><div className='card moneyCard'><h3>Finanzas mensuales</h3>{money.map(m=><div className='row' key={m[0]}><span>{m[0]}</span><b>{m[1]}</b></div>)}</div><div className='card ideaCard'><h3>Banco de ideas</h3><p className='muted'>YouTube, IA, negocios, trabajo y personal.</p><div className='chips'><span>🎥 Shorts</span><span>🤖 IA</span><span>💼 Trabajo</span></div><button className='ghost' onClick={openNewIdea}>Guardar idea</button></div></section>
    <section className='modules'><div className='card wide'><h3>Metas medibles</h3><p className='muted'>El progreso se calcula con valor actual ÷ valor objetivo.</p>{goals.map(g=><div className='goal' key={g.id}><div><b>{g.name}</b><span>{pct(g.current,g.target)}%</span></div><small>{Number(g.current).toLocaleString()} / {Number(g.target).toLocaleString()} {g.unit}</small><div className='bar'><i style={{width:pct(g.current,g.target)+'%'}}></i></div></div>)}</div><div className='card calendar'><h3>Calendario rápido</h3><div className='day'><b>Hoy</b><span>{todayTasks.length} tareas</span></div><div className='day'><b>Mañana</b><span>{tasks.filter(t=>t.due==='Mañana'&&!t.done).length} tareas</span></div><div className='day'><b>Semana</b><span>{tasks.filter(t=>t.due==='Esta semana'&&!t.done).length} tareas</span></div></div></section>
    <section className='modules'><div className='card wide'><h3>Proyectos activos</h3>{projects.map(p=><div className='goal' key={p[0]}><div><b>{p[0]}</b><span>{p[2]}</span></div><small>{p[1]}</small><div className='bar violet'><i style={{width:p[2]}}></i></div></div>)}</div><div className='card food'><h3>Plan de alimentación</h3><p>🍳 {meals[0]?.breakfast || 'Desayuno pendiente'}</p><p>🍗 {meals[0]?.lunch || 'Almuerzo pendiente'}</p><p>🍓 {meals[0]?.dinner || 'Cena pendiente'}</p><button className='ghost' onClick={()=>setActiveTab('Comidas')}>Ver comidas</button></div></section>
  </>;
  const taskPanel = <section className='taskBoard'><div className='card wide'><div className='sectionHeader'><div><h3>Tareas</h3><p className='muted'>Crea, edita, completa, filtra y elimina tus pendientes.</p></div><button className='btn' onClick={openNewTask}>+ Nueva tarea</button></div><div className='taskStats'><div><b>{tasks.length}</b><span>Total</span></div><div><b>{pendingTasks.length}</b><span>Pendientes</span></div><div><b>{completedTasks.length}</b><span>Completadas</span></div></div><div className='filters'>{filters.map(f=><button key={f} onClick={()=>setTaskFilter(f)} className={taskFilter===f?'active':''}>{f}</button>)}</div><div className='taskList'>{filteredTasks.length===0&&<div className='emptyState'>No hay tareas para este filtro.</div>}{filteredTasks.map(t=><div className={'taskCard '+(t.done?'done':'')} key={t.id}><button className='check' onClick={()=>toggleTask(t.id)}>{t.done?'✓':''}</button><div><b>{t.title}</b><small>{t.category} · {t.due}</small></div><span className={'priority '+(t.priority==='Alta'?'alta':t.priority==='Media'?'media':'baja')}>{t.priority}</span><button className='edit' onClick={()=>openEditTask(t)}>Editar</button><button className='delete' onClick={()=>deleteTask(t.id)}>Eliminar</button></div>)}</div></div></section>;
