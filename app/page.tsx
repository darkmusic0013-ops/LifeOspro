...
export default function Home(){
  ...
  const dashboard = <>
    <section className='hero'><div className='card heroCard'><div className='spark'>✨</div><span className='pill'>Enfoque de hoy</span><h1>Ejecuta tu día con claridad.</h1><p className='muted'>LifeOS convierte tus pendientes, metas, finanzas e ideas en una ruta visual para saber qué hacer ahora y qué viene después.</p><div className='grid'><div className='mini active'><b>{pendingTasks.length}</b><span>Pendientes</span></div><div className='mini'><b>{todayTasks.length}</b><span>Para hoy</span></div><div className='mini'><b>RD$8k</b><span>Ahorro</span></div><div className='mini'><b>{goals.length}</b><span>Metas</span></div></div></div><div className='card focus'><div className='focusTop'><h3>Prioridad inmediata</h3><span className='pulse'>En vivo</span></div>{topTasks.map((t,i)=><div className='row taskRow' key={t.id}><div className='rank'>{i+1}</div><div><b>{t.title}</b><small>{t.category} · {t.due}</small></div><span className={'priority '+(t.priority=='Alta'?'alta':t.priority=='Media'?'media':'baja')}>{t.priority}</span></div>)}</div></section>
    ...
    <section className='modules'><div className='card wide'><h3>Metas medibles</h3><p className='muted'>El progreso se calcula con valor actual ÷ valor objetivo.</p>{goals.map(g=><div className='goal' key={g.name}><div><b>{g.name}</b><span>{pct(g.current,g.target)}%</span></div><small>{g.current.toLocaleString()} / {g.target.toLocaleString()} {g.unit}</small><div className='bar'><i style={{width:pct(g.current,g.target)+'%'}}></i></div></div>)}</div><div className='card calendar'><h3>Calendario rápido</h3><div className='day'><b>Hoy</b><span>{todayTasks.length} tareas</span></div><div className='day'><b>Mañana</b><span>{tasks.filter(t=>t.due==='Mañana'&&!t.done).length} tareas</span></div><div className='day'><b>Semana</b><span>{tasks.filter(t=>t.due==='Esta semana'&&!t.done).length} tareas</span></div></div></section>
