...
const tabs = ['Dashboard','Misiones','Metas','Imperios','Planner','Ideas','Finanzas','Comidas','Life AI','Perfil'];
const empires = [
...
const aiActions = ['Organiza mi día','Crea plan para YouTube','Genera ideas virales','Redacta correo profesional','Convierte meta en tareas'];
...
export default function Home(){
  ...
  const LifeAIBlock = <div className='lifeAi card'><span className='pill'>Life AI</span><h3>¿Qué quieres organizar hoy?</h3><div className='aiGrid'>{aiActions.map(a=><button className='ghost' key={a}>{a}</button>)}</div></div>;

  const dashboard = <div className='controlCenter'>
    <section className='heroPanel card'><span className='pill'>Centro de control personal</span><h1>Organiza tu vida como un sistema.</h1><p className='muted'>Misiones, metas, imperios, dinero, comidas e ideas en un solo espacio diseñado para claridad mental.</p><div className='statGrid'><div><b>{todayTasks.length}</b><span>Misiones de hoy</span></div><div><b>{weeklyProgress}%</b><span>Progreso semanal</span></div><div><b>🔥 {Math.max(1, completedTasks.length + 1)}</b><span>Racha diaria</span></div><div><b>{goals.length}</b><span>Metas activas</span></div></div></section>
    <section className='missionCard card'><div className='sectionHeader'><div><span className='pill urgent'>Misión del Día</span><h3>{topTasks[0]?.title || 'Define tu misión principal'}</h3><p className='muted'>{topTasks[0] ? `${topTasks[0].category} · ${topTasks[0].due}` : 'Crea una misión para comenzar.'}</p></div><span className={'priority '+(topTasks[0]?.priority==='Alta'?'alta':topTasks[0]?.priority==='Media'?'media':'baja')}>{topTasks[0]?.priority || 'Nueva'}</span></div><button className='btn' onClick={openNewTask}>+ Nueva misión</button></section>
    <section className='modules'><div className='card'><h3>Qué debo hacer ahora</h3><p className='muted'>Próxima acción recomendada.</p><ol>{topTasks.slice(0,4).map(t=><li key={t.id}>{t.title}</li>)}</ol></div><div className='card'><h3>Finanzas mensuales</h3>{[['Salario',fmt(finance.salary)],['Gastos',fmt(totalExpenses)],['Disponible',fmt(balance)]].map(m=><div className='row' key={m[0]}><span>{m[0]}</span><b>{m[1]}</b></div>)}</div>{LifeAIBlock}</section>
    <section className='modules'><div className='card wide'><h3>Metas medibles</h3>{goals.map(g=><div className='goal' key={g.id}><div><b>{g.name}</b><span>{pct(g.current,g.target)}%</span></div><small>{Number(g.current).toLocaleString()} / {Number(g.target).toLocaleString()} {g.unit}</small><div className='bar'><i style={{width:pct(g.current,g.target)+'%'}} /></div></div>)}</div><div className='card'><h3>Calendario rápido</h3><div className='day'><b>Hoy</b><span>{todayTasks.length} tareas</span></div><div className='day'><b>Mañana</b><span>{tasks.filter(t=>t.due==='Mañana'&&!t.done).length} tareas</span></div><div className='day'><b>Semana</b><span>{tasks.filter(t=>t.due==='Esta semana'&&!t.done).length} tareas</span></div></div></section>
    <section className='modules'><div className='card wide'><h3>Mis Imperios</h3>{empires.map(p=><div className='empireRow' key={p[1]}><div><span className='empireIcon'>{p[0]}</span><b>{p[1]}</b><small>{p[2]}</small></div><span>{p[3]}</span></div>)}</div><div className='card'><h3>Banco de ideas</h3><p className='muted'>YouTube, IA, negocios, trabajo y personal.</p><div className='chips'><span>🎥 Shorts</span><span>🤖 IA</span><span>💼 Trabajo</span></div><button className='ghost' onClick={openNewIdea}>Guardar idea</button></div></section>
  </div>;
