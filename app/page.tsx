...
export default function Home(){
  ...
  useEffect(() => { try {
    const st = window.localStorage.getItem(taskStorageKey); const sg = window.localStorage.getItem(goalStorageKey); const sf = window.localStorage.getItem(financeStorageKey); const si = window.localStorage.getItem(ideaStorageKey); const sm = window.localStorage.getItem(mealStorageKey);
    if(st) setTasks(JSON.parse(st)); if(sg) setGoals(JSON.parse(sg)); if(sf) setFinance(JSON.parse(sf)); if(si) setIdeas(JSON.parse(si)); if(sm) setMeals(JSON.parse(sm));
  } catch {} }, []);
  ...
  const content = activeTab === 'Tareas' ? taskPanel : activeTab === 'Metas' ? goalPanel : activeTab === 'Finanzas' ? financePanel : activeTab === 'Ideas' ? ideaPanel : activeTab === 'Comidas' ? mealPanel : dashboard;
